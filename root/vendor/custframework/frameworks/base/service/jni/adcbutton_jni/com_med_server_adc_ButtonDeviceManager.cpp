/**
 * File: com_med_server_adc_ButtonDeviceManager.cpp
...
#define LOG_NDEBUG 0
#define LOG_TAG "libadc_button_jni"
...
static const char* const kClassName =
        "com/med/server/adc/ButtonDeviceManager";
        
static ADCButtonClient* pClient = nullptr;
...
ADCButtonCallback::ADCButtonCallback(){}
ADCButtonCallback::~ADCButtonCallback(){}

::android::binder::Status ADCButtonCallback::onButton(const ::android::String16& event) {
    ALOGV("Bn ADCButtonCallback : [%s] enter", __FUNCTION__);
    ALOGV("event: %s\n", String8(event).string());
    notifyJavaLayer(String8(event).string(), JNI_TRUE);
        return ::android::binder::Status::ok();
}

sp<ADCButtonCallback> ADCButtonCallback::getInstance() {
    ALOGV("Bn ADCButtonCallback : [%s] enter", __FUNCTION__);
    return new ADCButtonCallback();
}
} // namespace service
} // namespace button
} // namespace adc
...
init(JNIEnv *env, jobject thiz)
{
    ...
    pClient -> setADCButtonCallback(ADCButtonCallback::getInstance());
